...
DEFAULT_EVENT_TAXONOMY: dict[str, dict[str, list[str]]] = {
    "duration_type": {
...
class AppConfig:
    ...
    @property
    def min_prediction_score_threshold(self) -> float:
        return float(self.raw["strategy"].get("min_prediction_score_threshold", -0.01))

    @property
    def association_weights(self) -> dict[str, float]:
        defaults = {
            "direct_mention": 0.45,
            "business_match": 0.25,
        ...
        raw_weights = self.raw.get("scoring", {}).get("association", {})
        return {
    ...
    @property
    def association_weight_profiles(self) -> dict[str, dict[str, float]]:
        raw_profiles = self.raw.get("scoring", {}).get("association_profiles", {})
    ...
    @property
    def subject_bias_map(self) -> dict[str, float]:
        defaults = {
            "政策类事件": 1.08,
            "公司类事件": 1.12,
            "行业类事件": 1.0,
        ...
        raw_mapping = self.raw.get("scoring", {}).get("subject_bias", {})
