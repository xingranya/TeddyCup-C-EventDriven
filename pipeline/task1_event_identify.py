...
def run_event_identification(news_df: pd.DataFrame, event_taxonomy: dict[str, dict[str, list[str]]] | None = None) -> pd.DataFrame:
    ...
    for idx, row in news_df.iterrows():
        if idx in consumed:
            continue
        anchor_time: datetime = pd.Timestamp(row["published_at"]).to_pydatetime()
        anchor_text = f"{row['title']} {row['content']}"
        anchor_title = str(row['title'])
        anchor_entities = set(filter(None, str(row.get("entity_candidates") or "").split("、")))
        cluster = [idx]
        consumed.add(idx)
        for other_idx, other in news_df.iloc[idx + 1 :].iterrows():
            if other_idx in consumed:
                continue
            other_time: datetime = pd.Timestamp(other["published_at"]).to_pydatetime()
            ...
            shared_keywords = len(set(extract_all_keywords(anchor_text)) & set(extract_all_keywords(other_text)))
            other_entities = set(filter(None, str(other.get("entity_candidates") or "").split("、")))
    ...
    for cluster_indices in grouped_indices:
        cluster_df = news_df.loc[cluster_indices].copy().sort_values("published_at")
        evidence_text = "；".join(cluster_df["title"].tolist()[:3])
        aggregated_text = " ".join((cluster_df["title"] + " " + cluster_df["content"]).tolist())
        published_at = pd.Timestamp(cluster_df["published_at"].min()).to_pydatetime()
...
def choose_event_name(cluster_df: pd.DataFrame) -> str:
    ...
    # 过滤掉长度<8或>60字符的标题
    filtered_titles = [(title, score) for title, score in title_scores.items() if 8 <= len(title) <= 60]
