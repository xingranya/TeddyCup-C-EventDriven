...
- **观察窗口**：事件日前第 1 天 ~ 后第 10 天（用于输出完整 AR/CAR 路径）
- **统计窗口**：严格使用 `AR(+1)`、`CAR(0,2)`、`CAR(0,4)`，不把 `-1` 日混入统计口径

### 锚点交易日

- 交易日收盘前发布：锚定当日交易日
- 收盘后发布或非交易日发布：顺延到下一交易日
...
- `event_study_stats.csv` — 按事件聚合的统计量（样本数、均值 `AR(+1)`、均值 `CAR(0,2)`、均值 `CAR(0,4)`、标准差、正收益占比）
- `joint_mean_car.png` — 按正向/负向事件分组的均值 CAR 曲线

## 模型性能实验

为了对齐赛题中“自行设计实验分析所构建预测模型性能”的要求，周报现在会新增模型性能实验章节，默认统计：

- `car_4d` 方向准确率
- Top-K 命中率
- 预测分数与实现 `CAR(0,4)` 的秩相关
- 按事件主体类型分组的表现摘要

性能实验读取历史 `outputs/weekly/<week>/` 下的预测与实现样本，但只会纳入 `<= 当前 asof_date` 的周目录，避免旧周重跑时把未来周结果混进来形成前视偏差。
